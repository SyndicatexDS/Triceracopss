    ...
    async def help(self, ctx, commandName=None):
        ...
        if commandName is not None:
            ...
            if commandName2 is None:
                ...
                embed = discord.Embed(title=f"**{commandName2.name.upper()} COMMAND :**", description="[**GitHub**](https://playark.online/)", color=0xdeaa0c)
                ...
                embed.set_footer(text="Bot Created by Syndicate#0001")
                await ctx.channel.send(embed=embed)
        else:
            prefix = await getGuildPrefix(self.bot, ctx)
            embed = discord.Embed(title=f"__**Help page of {self.bot.user.name.upper()}**__", description="[**GitHub**](https://playark.online/)\n\n**{prefix}help (command) :**Display the help list or the help data for a specific command.", color=0xdeaa0c)
            embed.set_thumbnail(url=f'{self.bot.user.avatar_url}')
